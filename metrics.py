...
class Metric:
    def __init__(self,labels_list, predictions, anomaly_map_list, gt_list, category, config) -> None:
    ...
    def miscalssified(self):
        result_save_dir = os.path.join(os.getcwd(), self.config.metrics.result_dir, self.category, 'result.txt')
        with open(os.path.join(result_save_dir), 'a', encoding='utf-8') as file:
        ...
        for i,(l,p) in enumerate(zip(labels_list, predictions0_1)):
            if l != p:
                ...
                with open(os.path.join(result_save_dir), 'a', encoding='utf-8') as file:
                    file.write(f'| | Sample: {i:3d}\tpredicted as: {p.item()}\tlabel is: {l.item()}\n')
