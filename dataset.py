        ...
        if self.is_train:
            ...
            if self.config.data.mask:
                if os.path.dirname(image_file).endswith("good"):
                    ...
                    elif self.config.data.name == 'MVTec_LOCO':
                        directory, file_name = os.path.split(image_file.replace("/test/", "/ground_truth/"))
                        file_name_without_extension = os.path.splitext(file_name)[0]
                        new_image_file = os.path.join(directory, file_name_without_extension, '000.png')
                        target = Image.open(new_image_file)
                    else:
                        target = Image.open(
