...
def domain_adaptation(unet, category, config, fine_tune):
    ...
    if fine_tune:      
        ...
        for epoch in range(config.model.DA_epochs):
            ...
            torch.save(feature_extractor.state_dict(), os.path.join(os.path.join(os.getcwd(), config.model.checkpoint_dir), category,f'feat{epoch+1}'))
    else:
        ...
        checkpoint = torch.load(os.path.join(os.path.join(os.getcwd(), config.model.checkpoint_dir), category,f'feat{config.model.DA_chp}'))#{config.model.DA_chp}            
